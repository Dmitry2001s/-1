...
runService.Stepped:Connect(function()
    ...
    if char and char:FindFirstChild("Humanoid") then
        ...
        if states.noclip or states.flying then 
        ...
        if states.flying and root then 
            root.Velocity = (hum.MoveDirection.Magnitude > 0) and (workspace.CurrentCamera.CFrame.LookVector * states.flySpeed) or Vector3.new(0, 0.1, 0) 
        end
    end
end)
...
local function createESP(p)
    ...
    local function apply()
        ...
        hi.Name = "D_Hi"; hi.FillTransparency = 0.5; hi.OutlineTransparency = 0

        local bg = Instance.new("BillboardGui", head)
        bg.Name = "D_Name"; bg.Size = UDim2.new(0, 100, 0, 20); bg.AlwaysOnTop = true; bg.StudsOffset = Vector3.new(0, 3, 0)
        
        local tl = Instance.new("TextLabel", bg)
        tl.Size = UDim2.new(1, 0, 1, 0); tl.BackgroundTransparency = 1; tl.Font = "GothamBold"; tl.TextSize = 14
        ...
        task.spawn(function()
            while char and char.Parent and hi and hi.Parent do
                ...
                tl.Visible = states.espNames; tl.TextColor3 = c; tl.Text = p.DisplayName
                task.wait(0.1)
            end
        end)
    end
